import bitops
import types

type
    uint2* = range[0'u8..3'u8]
    uint3* = range[0'u8..7'u8]

func lsb*(value: uint16): uint8 {.inline.} = (value and 0xFF).uint8
func msb*(value: uint16): uint8 {.inline.} = (value shr 8).uint8
func concat*(lo: uint8, hi: uint16): uint16 {.inline.} = (hi shl 8) or lo

func x*(opcode: uint8): uint2 {.inline.} = opcode.bitsliced(6..7)
func y*(opcode: uint8): uint3 {.inline.} = opcode.bitsliced(3..5)
func z*(opcode: uint8): uint3 {.inline.} = opcode.bitsliced(0..2)
func p*(opcode: uint8): uint2 {.inline.} = opcode.bitsliced(4..5)
func q*(opcode: uint8): bool {.inline.} = opcode.testbit(3).bool

func signed*(n: uint8): uint16 = cast[int8](n).uint16

func isboundto*[T: SomeInteger](value, lower, upper: T): bool {.inline.} = 
    (value >= lower) and (upper >= value)

func checkHalfCarry*(a, b: uint8): bool {.inline.} = 
    (a and 0xF) + (b and 0xF) >= 0x10

func checkHalfBorrow*(a, b: uint8): bool {.inline.} = 
    (a and 0xF) < (b and 0xF)

func group2adjust*(reg: uint2): R16Type =
    if reg == 3: result = AF
    else: result = R16Type(reg)
